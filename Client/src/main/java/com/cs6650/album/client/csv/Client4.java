...
public class Client4 {
  ...
  public static void main(String[] args) throws InterruptedException, IOException {
    ...
    CountDownLatch latch1 = new CountDownLatch(INIT_THREADS);
...
//        displayStatistics(getLatencies);
    ...
    for (int i = 0; i < numThreads; i++) {
      executor.submit(() -> {
        for (int j = 0; j < requestsPerThread; j++) {
          sendRequest(serverURI + "/albums", "POST", true);

          // Send two like requests and one dislike request
          sendRequest(serverURI + "/review/like/1", "POST", false);
          sendRequest(serverURI + "/review/like/1", "POST", false);
          sendRequest(serverURI + "/review/dislike/1", "POST", false);
        }
        latch.countDown();
      });
    }
  ...
  private static void sendRequest(String urlString, String method, boolean isAlbumRequest) {
    int retries = 0;
    ...
    while (retries < 5 && !success) {
      ...
      try {
        ...
        if ("POST".equals(method)) {
          responseCode = sendPostRequest(urlString, isAlbumRequest);
        } else {  // For the GET method
          responseCode = sendGetRequest(urlString);
        }
...
//        System.out.println("GET latency: " + getLatency);
  ...
  private static int sendPostRequest(String urlString, boolean isAlbumRequest) throws Exception {
    HttpPost httpPost = new HttpPost(urlString);
    CloseableHttpResponse response;
    if (isAlbumRequest) {
      response = sendAlbumPostRequest(httpPost);
    } else {
      response = sendReviewPostRequest(httpPost);
    }
    try {
      HttpEntity entity1 = response.getEntity();
      EntityUtils.consume(entity1);
  ...
  private static CloseableHttpResponse sendReviewPostRequest(HttpPost httpPost) throws IOException {
    httpPost.setEntity(new ByteArrayEntity(new byte[0]));
    return POOLED_CLIENT.execute(httpPost);
  }

  private static CloseableHttpResponse sendAlbumPostRequest(HttpPost httpPost) throws Exception {
